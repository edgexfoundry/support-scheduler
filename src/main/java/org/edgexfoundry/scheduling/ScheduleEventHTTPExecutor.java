...
public class ScheduleEventHTTPExecutor {
	...
	private static final Logger logger = Logger.getLogger(ScheduleEventHTTPExecutor.class);
	...
	public void execute(final ScheduleEvent event) {
		...
		final String method = event.getAddressable().getMethod().toString();
		...
		try {
			if (url == null) {
				...
				// per original intent, defaulting to POST.  This was before a method existed in addressable.
